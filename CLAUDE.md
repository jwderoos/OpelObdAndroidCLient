...
- **Debug instrumentation is a toggle, not a `TEMPORARY` marker.** When adding diagnostic logging to chase a bug across sessions (USB/serial traces, protocol record dumps, connection state, etc.), gate it behind a persisted boolean debug setting (default off, surfaced in an in-app debug menu) instead of ad-hoc `Log.i(..., "TEMPORARY")`/`println` calls meant to be deleted once the bug is found. Goal: a future session can re-enable fine-grained capture on real hardware without recompiling and without re-inventing logging already written in a prior session. Modules without an Android dependency (e.g. `:core:transport`, `:core:protocol`) can't read this setting directly — plumb it in as an injected logger/flag from `:app` rather than adding an Android dependency to a pure-Kotlin module. Only delete instrumentation once it's no longer useful for *any* future debugging, not just the bug currently being chased.
## Commands
