...
/** One reported diagnostic trouble code: the raw 16-bit [code] and its status byte. */
data class Dtc(val code: Int, val status: Int)
    ...
    override fun decodeResponse(payload: ByteArray): Response {
        ...
        val dtcs = List(count) { index ->
            ...
            Dtc(
                ...
                status = payload[offset + 2].toInt() and 0xFF,
            )
        }
